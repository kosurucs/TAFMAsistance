...
export default function QuoteBar({ symbol, quote }) {
  if (!quote) {
    return (
      <div className="quote-bar loading">
        <span className="qb-symbol">{symbol}</span>
  ...
  return (
    <div className="quote-bar">
      ...
      <div className="qb-right">
        ...
        <div className="qb-stat"><span>H</span><strong style={{color:'#3fb950'}}>₹{Number(quote.high || 0).toFixed(2)}</strong></div>
        <div className="qb-stat"><span>L</span><strong style={{color:'#f85149'}}>₹{Number(quote.low || 0).toFixed(2)}</strong></div>
